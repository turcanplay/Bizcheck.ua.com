  ...
  useEffect(() => {
    ...
    adminApi.listTests()
      .then(({ tests }) => {
        ...
        setTest(t);
        if (!t) setError('Test not found');
      })
      .catch(e => setError(e.message));
  }, [slug]);

  if (error) return <div className="admin-error">⚠️ {error}</div>;
  if (!test) return <div className="admin-empty">Loading...</div>;

  return (
    <>
      <div className="admin-section-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          <Link to="/admin_bizcheck_md_crowe/tests" className="admin-back-link">← Back to tests</Link>
