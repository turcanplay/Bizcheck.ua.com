...
def _auto_slug(title_uk, title_en):
    base = (title_uk or title_en or "").lower().strip()
...
def create_template(slug, title_uk, title_en, description_uk="", description_en="",
    ...
    slug = (slug or "").strip().lower() or _auto_slug(title_uk, title_en)
    ...
    return _serialize(Template.create(
        slug, title_uk[:255], title_en[:255],
...
def update_template(template_id, data):
    ...
    return _serialize(Template.update(
        ...
        (data.get("title_en") or existing["title_en"]).strip()[:255],
