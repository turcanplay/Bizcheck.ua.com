...
class Question:
    @staticmethod
    def create(block_id, text_uk, text_en, note_uk=None, note_en=None, order_index=0, parent_question_id=None):
        return execute(
            """INSERT INTO questions (block_id, text_uk, text_en, note_uk, note_en, order_index, parent_question_id)
               VALUES (%s, %s, %s, %s, %s, %s, %s)
               RETURNING *""",
            (block_id, text_uk, text_en, note_uk, note_en, order_index, parent_question_id),
        )
    ...
    @staticmethod
    def update(question_id, block_id, text_uk, text_en, note_uk, note_en, order_index, parent_question_id=None):
        return execute(
            """UPDATE questions
               SET block_id = %s, text_uk = %s, text_en = %s,
                   note_uk = %s, note_en = %s, order_index = %s, parent_question_id = %s
               WHERE id = %s RETURNING *""",
            (block_id, text_uk, text_en, note_uk, note_en, order_index, parent_question_id, question_id),
        )
