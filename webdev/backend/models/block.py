...
class Block:
    @staticmethod
    def create(test_id, title_uk, title_en, order_index=0):
        """Create a new block for a given test with bilingual title."""
        return execute(
            """INSERT INTO blocks (test_id, title_uk, title_en, order_index)
               VALUES (%s, %s, %s, %s)
               RETURNING *""",
            (test_id, title_uk, title_en, order_index),
        )
    ...
    @staticmethod
    def update(block_id, title_uk, title_en, order_index, test_id=None):
        if test_id is not None:
            return execute(
                """UPDATE blocks
                   SET test_id = %s, title_uk = %s, title_en = %s, order_index = %s
                   WHERE id = %s RETURNING *""",
                (test_id, title_uk, title_en, order_index, block_id),
            )
        return execute(
            """UPDATE blocks SET title_uk = %s, title_en = %s, order_index = %s
               WHERE id = %s RETURNING *""",
            (title_uk, title_en, order_index, block_id),
        )
