...
class TestSalesNotify:
    ...
    def _setup(self, monkeypatch, sub, *, claim_ok=True, configured=True):
        ...
        # Assert on the REAL seam: sends/updates are handed to a single serialized
        # queue worker via _enqueue (not a thread-per-lead). Capturing _enqueue is
        # order-independent, unlike patching threading.Thread (the worker starts
        # once per process, so a Thread patch only fires for the first test).
        monkeypatch.setattr(sn, "_ensure_worker", lambda: None)
        monkeypatch.setattr(sn, "_enqueue", lambda job, **kw: sent.append(job))
        return sn, sent
